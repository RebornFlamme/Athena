 ...
 * Géo-ancrée sur une ORIGINE passée à la création (repère ENU local) : ici le 1er
 * objet géolocalisé réel (les vraies instances de l'agent), plus le mock d'origine.
 *
 * Repère local (après scale (echelle, −echelle, echelle)) : X = est, Y = nord,
 * Z = haut. Un engin au cap 0 pointe vers le nord (+Y).
 */

/** Sous-types d'engin (dimensions du pavé). */
...
/** Texture d'un nom (pastille blanche sur rouge) pour l'étiquette toit. */
...
export function creerCoucheEngins(id: string, origine: [number, number]): CoucheEngins {
  ...
  // Géométrie de pavé mise en cache PAR TYPE ; matériaux partagés.
  ...
  function creerNoeud(e: EtatEngin): Noeud {
    ...
    // Personnels : petits cubes en file derrière l'engin.
    ...
    // Étiquette « sur le toit » : plan horizontal, ajouté à la SCÈNE → ne tourne pas.
  ...
  return {
    ...
    majEngins(engins: EtatEngin[], opts: OptionsEngins) {
      ...
      for (const e of engins) {
        ...
        const zBase = e.aerien ? ALT_VOL : 0
        n.group.position.set(est, nord, zBase)
        n.group.rotation.z = (-e.cap * Math.PI) / 180

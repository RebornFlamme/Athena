...
**En cours :** **Créateur de simulation** + **dashboard flux audio**. Code complet, `npm run build` vert. Test runtime bloqué par les migrations `0002` + `0003`.
**Dashboard (`/tableau-de-bord/:id`) refondu :** panneaux **redimensionnables** shadcn (`ui/resizable.tsx`) — **carte à gauche**, **flux audio à droite** (`PanneauFluxAudio.tsx`). Chaque flux = un appel avec un **histogramme voix live** (`VisualiseurVoix.tsx`, Web Audio `AnalyserNode` FFT). **Lecture muette par défaut** ; **clic sur un flux = bascule l'écoute** (relie l'analyser à la sortie). Moteur Web Audio dans `useSimulationPlayback.ts` (état `actifs`/`ecoutes`, `basculerEcoute`, `getAnalyseur`). `MainCourante` retirée de cette vue (fichier conservé).

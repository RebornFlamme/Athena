...
- **Dashboard `/tableau-de-bord`** (`components/dashboard/DashboardPage.tsx`, page directe) = `ResizablePanelGroup` (`ui/resizable.tsx`, dep `react-resizable-panels`) : **carte** (`dashboard/Carte.tsx`, gauche) + **`PanneauFluxAudio.tsx`** (droite). Chaque flux : titre + visualiseur + toggle écoute (`Volume2`/`VolumeX`). **Visualiseur du Live feed = `VisualiseurAudioLive.tsx`** (dep **`react-audio-visualize`** → `LiveAudioVisualizer`, barres FFT) : crée un `MediaRecorder` à partir de `getStream(appelId)`, monté/démonté avec le flux affiché (⚠ le composant crée **1 `AudioContext` par instance** → borner aux appels réellement actifs limite le risque de la limite navigateur ~6 ; repli sur `VisualiseurVoix` si `MediaRecorder` indisponible). `VisualiseurVoix` (histogramme maison piloté par `getByteFrequencyData`) reste comme **repli** de `VisualiseurAudioLive` ; ⚠ **`FeuilleTranscription` (volet ouvert) utilise désormais `VisualiseurAudioLive`** — le MÊME visualiseur que le live feed fermé (cohérence demandée). **Carte en 3D** (`Carte.tsx`) : `maxPitch:70` + `NavigationControl` avec compas/`visualizePitch` ; sur `m.on('load')`, `ajouterBatiments3D` ajoute une source vectorielle **BD TOPO® IGN** (`data.geopf.fr/tms/1.0.0/BDTOPO`, sans clé) et une couche `fill-extrusion` (source-layer `batiment`) dont la **hauteur = champ `hauteur`** réel (poussée entre z14→z15.5, dégradé selon la hauteur, sous les libellés). Le recentrage sur une entité géolocalisée passe en **`flyTo` incliné** (`zoom:16, pitch:55`) → les immeubles apparaissent en volume. ⚠ IGN limite à **1 req/s** : sur une session de dev très sollicitée, le fond peut cesser de tuiler (carte sombre) — indépendant du code, OK en usage normal.
- **Refonte visuelle dashboard (branche `ux`, mode clair)** : (a) **fond quadrillage** plein cadre (`components/ui/tiles.tsx`, dep **`framer-motion`**, var `--tile`) posé derrière les panneaux ; dockview rendu **transparent** (`--dv-*background-color` dans `.dv-athena .dv-shell`) pour laisser transparaître la grille ; (b) **cartes du panneau Objets en glassmorphism** (`bg-white/70 backdrop-blur-md border-white/50 shadow-lg`, panneau `bg-transparent`). (c) **Timeline de rejeu** (`components/dashboard/Timeline.tsx`, au-dessus des objets, façon montage vidéo) : store **`store/useTimeline.ts`** (curseur epoch ms, bornes `t0/tMax` calculées depuis les `cree_le` des `object_instances`+`agent_journal`, mode live `suit`, `play` rejoue le run en ~12s). Hooks **`hooks/useScrub.ts`** (`useInstancesScrub`/`useEditsScrub`) filtrent `cree_le <= curseur` (tout si live) et sont branchés sur **carte, objets, couche sémantique et trajets d'engins** → scruber en arrière retire les entités/évènements/animations postérieurs, avancer les rejoue. `useTrajetsEngins` **cache les tracés** par intervention (pas de re-fetch Overpass/IGN au scrub). Playhead à logique pointer pure (comme `TimelineMontage`).

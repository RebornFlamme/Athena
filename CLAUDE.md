...
- **Transcription live (Sheet)** : un **clic sur un flux** du live feed ouvre `dashboard/FeuilleTranscription.tsx` (shadcn `Sheet` à droite, `side="right"`, `sm:max-w-lg`) = même carte audio + transcript en direct. L'icône volume reste un bouton isolé (`stopPropagation`) pour le toggle écoute ; le reste de la carte ouvre le volet. Tech = **branche `stt` du teammate**, intégrée verbatim : backend Python `poc-stt/` (FastAPI `/ws` → Google Speech-to-Text V2 **Chirp 3**, `language_codes=["auto"]`, `interim_results`). Côté front, `lib/sttStream.ts` (`SttSession`) porte le chemin « lecture fichier » du POC : `fetch(audio_url)` (Supabase) → `decodeAudioData` → chunks 80 ms → downsample **16 kHz mono Int16 (LINEAR16)** → WebSocket binaire. **Session muette** (jamais reliée à la sortie ; l'audible reste géré par `useSimulationPlayback`). Le transcript démarre à **t=0** du fichier (pas synchronisé à la position de lecture de la sim — limite POC assumée). Hook `hooks/useTranscription.ts` (interim + `finals[]` + langue). URL WS configurable via `VITE_STT_WS_URL` (défaut `ws://localhost:8000/ws`), état d'erreur « serveur STT injoignable » gracieux. **Backend à lancer à part** (`cd poc-stt && uvicorn main:app --port 8000` + creds GCP `GOOGLE_APPLICATION_CREDENTIALS`/`GOOGLE_CLOUD_PROJECT`, région `us`). WS cross-origin (Vite:5173 → :8000) OK sans CORS (Starlette accepte le handshake).

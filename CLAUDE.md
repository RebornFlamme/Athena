...
- **Dashboard `/tableau-de-bord/:id`** = `ResizablePanelGroup` (`ui/resizable.tsx`, dep `react-resizable-panels`) : **carte** (gauche) + **`PanneauFluxAudio.tsx`** (droite). Chaque flux : titre + `VisualiseurVoix` (barres pilotées par `getByteFrequencyData`, repli synthétique si pas d'analyser) + toggle écoute (`Volume2`/`VolumeX`). `MainCourante` retirée de cette vue (fichier conservé, réutilisable).

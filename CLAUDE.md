...
- ⚠ **`CarteEngagement` est branchée au RÉEL et rend les vrais objets avec les ARTEFACTS 3D du collègue** (refactor juil. 2026) : garde la base `CarteLibre` et, pour chaque `object_instance` géolocalisée (`useInstancesDB`, live Realtime), choisit le rendu selon son `type_name` — **`categorie()`** dans `CarteEngagement.tsx` : contient `victim|bless|impliqu` → **dôme bleu** (`VictimesCustomLayer.majVictimes`) ; contient `engin|véhic|vehicle|appliance|truck|camion|pompe|échelle|ambulance|fpt|epa|bea|vsav|vlr` → **pavé rouge 3D** (`EnginsCustomLayer.majEngins`, sous-type FPT/EPA/… inféré du libellé) ; **sinon marqueur** coloré par statut. Plus **trajets d'engins réels** (`useTrajetsEngins`) + **caméra qui plonge** sur le 1er objet localisé. ⚠ **Les 2 couches custom three.js `EnginsCustomLayer`/`VictimesCustomLayer` ont été RESTAURÉES et rendues DYNAMIQUES** (refactor : `creer…(id, origine)` prend une **origine ENU** = le 1er objet géoloc réel, au lieu du point mock ; `VictimesCustomLayer` a gagné `majVictimes()` pour être alimentée en direct). Champs lus dans `fields` : engin `effectif|personnels|crew`, victime `nombre|persons|effectif` + `etage|floor|niveau`. **Restent supprimés** : `PanneauEngins`, `donneesMock`, `victimesMock`, `carte/zonesTactiques` (le mock). **Test facile** : le toggle « mock data » (fiole) insère des types `Victim`/`Appliance`/`Building`… → exerce dôme/pavé/marqueur. `CarteLibre` **réduite à l'essentiel** (juil. 2026) : ne restent en overlay que **2D/3D + sélecteur de thème** ; les contrôles/fonctions liés au scénario Peletier — **Floors** (découpe d'étages) et **Interactive 3D** (bâtiment éclaté) — ont été retirés, et le dossier **`components/batiment/` supprimé** (`EtagesTroisD`, `EtagesCustomLayer`, `batiment.ts`) → **`three`/`@types/three` ne sont plus tirés** (bundle allégé ; les deps restent déclarées, inutilisées). Le fond 3D (extrusion `public/batiments_peletier.json`, conservé) reste piloté par le bouton 2D/3D. ⚠ `CarteLibre` est encore **centrée en dur sur Paris-Peletier** → les immeubles 3D individuels ne s'affichent qu'à cet endroit ; ailleurs on a le fond + les marqueurs (la caméra suit quand même les vraies données).
